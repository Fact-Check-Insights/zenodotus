...
class MediaSourceTest < ActiveSupport::TestCase
  ...
  test "Can extract media post URLs from Archive.org addresses" do
    ...
    archive_to_direct_url_map.each  do |archive_url, post_url|
      assert_equal post_url, MediaSource.extract_post_url_if_needed(archive_url), initiated_from: Scrape.initiated_froms[:site]
    end
  end
  ...
  test "Can extract and fix media post URL from Archive.org addreses" do
    assert_equal "https://mobile.twitter.com/dirvenkatesh/status/1625066368579297280", MediaSource.extract_post_url_if_needed("https://web.archive.org/web/20230221054655/https:/mobile.twitter.com/dirvenkatesh/status/1625066368579297280"), initiated_from: Scrape.initiated_froms[:site]
  end

  test "Can archive media using archive.org URLs" do
    assert_not_nil Sources::Tweet.create_from_url!("https://web.archive.org/web/20220930151009/https://twitter.com/AmtrakNECAlerts/status/1397922363551870990", initiated_from: Scrape.initiated_froms[:site])
  end
  ...
  test "archive.org URL extraction does not affect normal URLs" do
    ...
    assert_equal tweet_url, MediaSource.extract_post_url_if_needed(tweet_url), initiated_from: Scrape.initiated_froms[:site]
  end
end
