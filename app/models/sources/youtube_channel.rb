...
private
  ...
  def self.youtube_channel_hash_from_youtube_archiver_channel(youtube_archiver_channel)
    ...
    if youtube_archiver_channel["aws_profile_image_key"].present?
    ...
    elsif youtube_archiver_channel["channel_image_file"].present?
    ...
    hash_to_return[:channel_image] = File.open(profile_image_path, binmode: true) if profile_image_path.present?
    hash_to_return
  end
end
