module Scrapable
  ...
  module ClassMethods
    ...
    # @params private whether the resulting ArchiveItem should be private
    # returns ScrapeJob
    sig { params(url: String, user: T.nilable(User), media_review: T.nilable(MediaReview), private: T.nilable(T::Boolean)).returns(ScrapeJob) }
    def create_from_url(url, user = nil, media_review: nil, private: nil)
      ...
      scrape = Scrape.create!({ url: url, scrape_type: scrape_type.serialize, media_review: media_review, user: user, private: private })
      scrape.enqueue
    end
  end
end
